            ...
            if (apiUser === "ajisaq" && apiKey === "1234567890") {
                ...
                try {
                    raw = await API.graphql(
                        graphqlOperation(queries.getSeller, {
                            id: username
                        })
                    );
                } catch (errors) {
                ...
                if (raw.data.getSeller !== null &&
                    raw.data.getSeller.username === username &&
                    raw.data.getSeller.password === password) {
                    res.setHeader("Content-Type", "application/json");
                    res.end(JSON.stringify({ status: "Ok", data: raw.data.getSeller }));
                } else {
                    res.setHeader("Content-Type", "application/json");
                    res.end(
